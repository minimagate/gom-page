...
{{ plot(path="charts/negation-embedding-pca.html", ratio="700 / 650", title="All logical states in a shared PCA plane", caption="Figure 5. Shared two-dimensional PCA of all 600 run embeddings. PC1 explains 9.8% and PC2 6.7% of variance (16.5% combined). PCA was fitted on the complete run population; grey tones encode logical state and marker shapes encode language. The plot is a lossy global orientation view and must not be used to read exact pairwise similarity, direction alignment, or closure.") }}
